...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'enquiries' | 'projects' | 'blogs' | 'services' | 'settings'>('enquiries');
  ...
  const handleSaveService = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      faqs: [
        ...
        { question: "Is soil stress analysis included?", answer: "Yes, borehole soil testing is completed before layout modeling starts." }
      ],
    ...
    if (isStaticMode) {
      ...
      try {
        ...
        if (editingItemId) {
          ...
          const newItem = {
            ...payload,
            id: serviceForm.title.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + Date.now()
          };
          items.push(newItem);
        }
    ...
    if (isStaticMode) {
      ...
      try {
        ...
        if (editingItemId) {
          ...
          const newItem = {
            ...payload,
            id: projectForm.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + Date.now()
          };
              ...
              <button
                onClick={() => {
                  ...
                  setServiceForm({
                    ...
                    features: 'Soil Boring Test, Cement Certifications, Fe 550 Rods',
                    faqs: ''
                  });
                }}
                className="px-4 py-2 bg-[#1A6DB5] hover:bg-[#1558a0] text-white text-xs font-bold rounded-xl flex items-center gap-1"
              ...
              {editingItemType === 'service' && (
                <form onSubmit={handleSaveService} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input type="text" required value={serviceForm.features} onChange={(e) => setServiceForm({ ...serviceForm, features: e.target.value })} placeholder="Borehole test, Tata TMT, Kajaria Tiles" className="border border-slate-200 px-4 py-2 text-xs rounded-xl" />
                  </div>
